...
app.get('/home',(req,res)=>{
    if(req.session.userid!=""){
        ...
        db.query(sql,[req.session.userid,req.session.userid],function(err,result){
            if(err)
                throw err;
            res.render("home",{data:"user tweet will be displayed",msg:msg,result:result,currentUser: req.session.user});
        });
    }
    else{
...
app.get('/profile_page', function(req, res){
    ...
    let sql1 ="select * from user where uid in (select uid from user_following where follow_uid=?)";
    db.query(sql1, [req.session.userid], function (err, result) {
      if (err) throw err;
      followers = result;
    });
    let sql2 =
      "select * from user where uid in (select follow_uid from user_following where uid=?)";
  
    db.query(sql2, [req.session.userid], function (err, result) {
      if (err) throw err;
      following = result;
      // console.log(result);
    });
    db.query(sql, [req.session.userid], function (err, result) {
      res.render('profile_page', {result: result,followers: followers,following: following,currentUser: req.session.user});
    });
  });
...
app.get('/change_password',(req,res)=>{
    ...
    res.render('change_password.ejs',{msg:pass})
})
